...
import { SelectObject, SelectString } from 'comunes/controles/select';
...
import { useTrabajadoresTextoStore } from 'modulos/trabajadores/store/StoreMensajes';
import {
  guardarDatos,
...
const Ocupacional = () => {
  ...
  const { tocoNacimiento, tocoIngreso } = useTrabajadoresTextoStore();
    ...
    <fieldset className={estilos.contenedor_seccion}>
      ...
      <section className={estilos.contenedor_preguntas}>
        ...
        <div className={estilos.fecha}>
          ...
          <MensajeFechas
            cumple={
              tocoIngreso && fechaIngresoCargo
                ? `Antigüedad: ${obtenerAntiguedad(
                  fechaIngresoCargo,
                  fechaRetiro,
                )}`
                : ''
            }
            noCumple="Por favor registre la fecha de nacimiento antes de colocar la fecha de ingreso"
            condicion={tocoNacimiento && fechaNacimiento !== ''}
          />
        </div>
        ...
        <div className={estilos.centrar}>
          <Radio label="Estado del trabajador" options={['Activo', 'Inactivo']} onChange={() => ''} />
        </div>
        <div className={estilos.fecha}>
          <Date
